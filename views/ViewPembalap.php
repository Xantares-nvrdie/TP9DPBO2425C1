...
class ViewPembalap implements KontrakView{
    ...
    public function tampilFormPembalap($data = null): string {
        ...
        if ($data) {
            $template = str_replace('name="action" value="add" id="pembalap-action" data-field', 'name="action" value="edit" id="pembalap-action" data-field', $template);
            $template = str_replace('value="" id="pembalap-id" data-field', 'value="' . htmlspecialchars($data['id']) . '" id="pembalap-id" data-field', $template);
